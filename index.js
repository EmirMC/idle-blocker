...
const idleBlock = (idleTime = maxIdleTime) => {
  console.log(
    `[${new Date().toLocaleString()}] Idle Time: ${Math.floor(
      desktopIdle.getIdleTime() / 60
    )}m ${Math.floor(desktopIdle.getIdleTime() % 60)}s`
  );
  if (desktopIdle.getIdleTime() < idleTime) {
    setTimeout(() => idleBlock(idleTime), (idleTime - desktopIdle.getIdleTime()) * 1000);
    return;
  }
  ...
  setTimeout(() => idleBlock(idleTime), idleTime * 1000);
  moveCount++;
};
// idleBlock();
module.exports = idleBlock;
